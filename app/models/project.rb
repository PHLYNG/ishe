class Project < ApplicationRecord
  ...
  def project_exists
    if self.check_project_exists == true
      return (Project.select{ |proj| proj.projeect_type == self.project_type } && Project.select{ |proj| proj.street1.to_f.between?((self.street1.to_f - 0.0002), (self.street1.to_f + 0.0002))} && Project.select{ |proj| proj.street2.to_f.between?((self.street2.to_f - 0.0002), (self.street2.to_f + 0.0002))})
    else
      return []
    end
  end
  ...
  def check_project_exists
    ...
    (Project.select{ |proj| proj.projeect_type == self.project_type }.count > 0 && Project.select{ |proj| proj.street1.to_f.between?((self.street1.to_f - 0.0002), (self.street1.to_f + 0.0002))}.count > 0 && Project.select{ |proj| proj.street2.to_f.between?((self.street2.to_f - 0.0002), (self.street2.to_f + 0.0002))}.count > 0)
  end
